...
void* Reactor::startReactor() {
    running = true;
    std::cout << "[Reactor] Reactor started" << std::endl;
    while (running) {
        ...
        if (activity < 0) {
            perror("select");
            std::cerr << "[Reactor] Error in select()" << std::endl;
            return nullptr;
        }
        ...
        for (int fd = 0; fd <= max_fd; ++fd) {
            if (FD_ISSET(fd, &read_fds)) {
                ...
                if (it != handlers.end()) {
                    std::cout << "[Reactor] Handling activity on fd " << fd << std::endl;
...
int Reactor::addFdToReactor(int fd, std::function<void(int)> func) {
    if (handlers.find(fd) != handlers.end()) {
        std::cerr << "[Reactor] Error: fd " << fd << " already exists in reactor" << std::endl;
    ...
    std::cout << "[Reactor] Added fd " << fd << " to reactor" << std::endl;
    return 0;
}
...
int Reactor::removeFdFromReactor(int fd) {
    if (handlers.find(fd) == handlers.end()) {
        std::cerr << "[Reactor] Error: fd " << fd << " not found in reactor" << std::endl;
    ...
    std::cout << "[Reactor] Removed fd " << fd << " from reactor" << std::endl;
    return 0;
}
...
pthread_t Reactor::startProactor(int sockfd, void* (*proactorFunc)(void*)) {
...
void Reactor::handleClient(int clientSocket, Graph& graph) {
    char buffer[1024];
