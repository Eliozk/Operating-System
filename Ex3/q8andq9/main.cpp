...
void* proactorFunc(void* sockfdPtr) {
    ...
    reactor.handleClient(clientSocket, graph); // Call handleClient on reactor instance
...
int main() {
    ...
    // Add the server socket to the reactor to accept new connections
    ...
    // Start the reactor to handle events on registered file descriptors
    std::cout << "[Server] Starting reactor to handle events..." << std::endl;
